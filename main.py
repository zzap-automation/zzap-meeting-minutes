...
LLM_MODEL    = "meta-llama/llama-4-scout-17b-16e-instruct"  # 30K free-tier TPM vs 12K on llama-3.3-70b
...
async def _post_with_retry(client, url, max_attempts=3, **kwargs):
    """POST with automatic retry on transient errors (502/503/504 gateway
    errors, which are typically momentary blips from the upstream provider's
    edge/CDN rather than genuine outages)."""
    last_resp = None
...
async def _compress_audio(data: bytes, filename: str, bitrate: str = "32k") -> bytes:
    """Compress audio to a mono MP3 at the given bitrate using ffmpeg.
    16 kHz / 32 kbps mono is good quality for speech and keeps file size low
    (~14 MB per hour of audio)."""
    with tempfile.NamedTemporaryFile(
        suffix=os.path.splitext(filename or ".mp3")[1] or ".mp3", delete=False
    ) as src_f:
        src_f.write(data)
        src_path = src_f.name
...
def _is_size_error(resp_text: str) -> bool:
    """Detect a Groq 'file/request too large' error regardless of exact
    status code or account tier."""
    text = resp_text.lower()
    return "request_too_large" in text or "request entity too large" in text or "too large" in text
...
async def transcribe(audio: UploadFile = File(...)):
    ...
    Pre-emptively compresses very large files (>90 MB) before the first
    attempt. If Groq still rejects the file as too large (which happens if
    the account is on the free tier's lower limit even though our own
    threshold assumes paid tier), it automatically compresses further and
    retries once — so the tool keeps working correctly regardless of which
    Groq tier is currently active.
    """
    data = await audio.read()
    PRE_COMPRESS_THRESHOLD = 90 * 1024 * 1024  # pre-emptive compression above this

    if len(data) > PRE_COMPRESS_THRESHOLD:
        try:
            data = await _compress_audio(data, audio.filename, bitrate="32k")
        except Exception as e:
            raise HTTPException(500, f"Compression failed: {e}")

    ext = os.path.splitext(audio.filename or ".mp3")[1] or ".mp3"

    async def _attempt(payload: bytes, filename_ext: str):
        async with httpx.AsyncClient(timeout=300) as client:
            return await _post_with_retry(
                client,
                f"{GROQ_BASE}/audio/transcriptions",
                headers=_groq_headers(),
                files={"file": (f"audio{filename_ext}", payload, "audio/mpeg")},
                data={"model": STT_MODEL, "response_format": "text"},
            )

    resp = await _attempt(data, ext)

    # Reactive fallback: if Groq rejects for size (e.g. account is on free
    # tier's lower limit), compress harder and retry once.
    if not resp.is_success and _is_size_error(resp.text):
        try:
            data = await _compress_audio(data, audio.filename, bitrate="24k")
        except Exception as e:
            raise HTTPException(500, f"File too large and compression failed: {e}")
        resp = await _attempt(data, ".mp3")
...
def _is_tpm_error(resp_text: str) -> bool:
    """Detect a Groq tokens-per-minute rate limit error, which happens when
    the account's active tier (free or paid) has a lower TPM cap than the
    request needs — regardless of which tier we assume is active."""
    text = resp_text.lower()
    return "rate_limit_exceeded" in text or "tokens per minute" in text or ("tpm" in text and "limit" in text)
...
async def generate(
    ...
    Sends the full transcript first for maximum quality. If Groq rejects it
    for exceeding the account's current tokens-per-minute limit (which can
    happen on the free tier, or if a paid upgrade hasn't taken effect yet),
    it automatically retries with a truncated transcript and a lower token
    budget — so generation still succeeds either way.
    """
    prompt = SYSTEM_PROMPT_DETAILED if detail_level == "detailed" else SYSTEM_PROMPT

    def _build_user_msg(text: str, truncated_note: bool = False) -> str:
        msg = "Please produce meeting minutes from this transcript.\n"
        if title:     msg += f"\nMeeting title: {title}"
        if attendees: msg += f"\nAttendees: {attendees}"
        if date:      msg += f"\nDate: {date}"
        if truncated_note:
            msg += "\n\nNote: transcript was shortened due to processing limits. Cover all sections present."
        msg += f"\n\nTRANSCRIPT:\n{text}"
        return msg

    async def _attempt(user_msg: str, max_tokens: int):
        async with httpx.AsyncClient(timeout=300) as client:
            return await _post_with_retry(
                client,
    ...
    max_tokens = 9000 if detail_level == "detailed" else 6000  # leaves headroom for input under 30K combined TPM
    resp = await _attempt(_build_user_msg(transcript), max_tokens)

    # Reactive fallback: TPM limit hit — truncate transcript and reduce
    # token budget, then retry once.
    if not resp.is_success and _is_tpm_error(resp.text):
        short_transcript, _ = _truncate_transcript(transcript, max_chars=16000)
        resp = await _attempt(_build_user_msg(short_transcript, truncated_note=True), min(max_tokens, 4000))
...
async def query_minutes(
    question: str = Form(...),
    series: str   = Form(""),
    history: str  = Form("[]"),   # JSON array of {role, content} pairs
):
    ...
    try:
        ...
        fts_result = q.text_search("search_vector", question).limit(12).execute()
    ...
    system = (
        ...
        "like 'that', 'who', 'when', or 'tell me more'."
    )

    # Parse conversation history
    import json as _json
    try:
        prior_messages = _json.loads(history) if history else []
        # Sanitise — only keep role/content, max 20 prior turns
        prior_messages = [
    ...
    # First user message carries the archive context; subsequent ones are plain questions
    if prior_messages:
        # Archive context already provided in first exchange — just ask the follow-up
        new_user_msg = question
    else:
        new_user_msg = "Meeting minutes archive:\n\n" + context + "\n\nQuestion: " + question

    messages = [{"role": "system", "content": system}]
    if not prior_messages:
        messages.append({"role": "user", "content": new_user_msg})
    else:
        # Inject archive context into the first user message if not already there
        first = prior_messages[0]
        if "Meeting minutes archive:" not in first.get("content", ""):
            first = {"role": "user", "content": "Meeting minutes archive:\n\n" + context + "\n\n" + first["content"]}
        messages.append(first)
        messages.extend(prior_messages[1:])
        messages.append({"role": "user", "content": new_user_msg})

    async def _attempt(msg_list, max_tokens):
    ...
    resp = await _attempt(messages, 3000)

    # Reactive fallback: TPM limit hit — retry with fewer source meetings
    # and a smaller token budget.
    if not resp.is_success and _is_tpm_error(resp.text):
        trimmed_relevant = relevant[:4]
        trimmed_context_parts = []
        for m in trimmed_relevant:
            title     = m.get("title", "Untitled")
            date      = m.get("date", "unknown")
            attendees = m.get("attendees", "")
            body      = m.get("minutes_md", "")[:3000]  # cap each meeting's content
            header    = "Meeting: " + title + " | Date: " + date + " | Attendees: " + attendees
            trimmed_context_parts.append("--- " + header + " ---\n" + body)
        trimmed_context = "\n\n".join(trimmed_context_parts)

        if prior_messages:
            first = {"role": "user", "content": "Meeting minutes archive:\n\n" + trimmed_context + "\n\n" + prior_messages[0]["content"]}
            fallback_messages = [{"role": "system", "content": system}, first] + prior_messages[1:] + [{"role": "user", "content": new_user_msg}]
        else:
            fallback_messages = [
                {"role": "system", "content": system},
                {"role": "user", "content": "Meeting minutes archive:\n\n" + trimmed_context + "\n\nQuestion: " + question},
            ]
        resp = await _attempt(fallback_messages, 1500)
        relevant = trimmed_relevant
