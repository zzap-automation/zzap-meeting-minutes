...
def _md_to_docx(md: str) -> BytesIO:
    ...
    # Set default view to print layout (single page) so Word doesn't open in
    # multi-page / web layout mode
    try:
    ...
    def flush_table():
        ...
        # ── merge consecutive same-value cells in column 0 (Agenda) ──
        if len(tbl.rows) > 2:
            groups, grp_start = [], 1
            grp_text = tbl.rows[1].cells[0].paragraphs[0].text.strip()
            for ri in range(2, len(tbl.rows)):
                ct = tbl.rows[ri].cells[0].paragraphs[0].text.strip()
                if ct != grp_text:
                    if ri - grp_start > 1:
                        groups.append((grp_start, ri - 1))
                    grp_start, grp_text = ri, ct
            last = len(tbl.rows) - 1
            if last - grp_start >= 1:
                groups.append((grp_start, last))
